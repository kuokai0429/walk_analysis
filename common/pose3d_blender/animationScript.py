...
def load_subject_keypoints(subject, subjects_annot_filepath):
...
#    subject_annot = annot_df.loc[annot_df['subject'] == subject]
    ...
    # subject_kp = subject_keypoints[int(subject_annot[column_name['start']]):int(subject_annot[column_name['end']])]
    subject_kp = subject_keypoints

    print(subject_kp.shape)
...
if __name__ == "__main__":
    ...
    os.chdir(r"C:\Users\user\Desktop\walk_analysis_v2")
    print(os.getcwd())
    ...
    s1_kp = load_subject_keypoints("1-V2-front", subjects_annot_filepath)
    ...
    for i in range(len(s1_kp[0])):
        
        loc = (s1_kp[0][i][0]*15, s1_kp[0][i][1]*15, s1_kp[0][i][1]*15)
    ...
    for i in range(len(connections)):
        ...
        kp1 = np.array([s1_kp[0][connections[i][0]][0]*15, 
            s1_kp[0][connections[i][0]][1]*15, s1_kp[0][connections[i][0]][1]*15])
        kp2 = np.array([s1_kp[0][connections[i][1]][0]*15, 
            s1_kp[0][connections[i][1]][1]*15, s1_kp[0][connections[i][1]][1]*15])
    ...
    start_frame = 1
    for i in range(0, len(s1_kp[0])):
        cubes[i].keyframe_insert("location", frame=start_frame)
    

    ## Change the location of the cubes
    
    for i in range(len(s1_kp)):
        for j in range(len(s1_kp[0])):
            
            changeCubeLocation(cubes[j], s1_kp[i][j][0]*15, s1_kp[i][j][1]*15, s1_kp[i][j][2]*15, i)